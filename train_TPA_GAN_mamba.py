...
# initial setup
MODEL_PATH = './Generator_save/Generator_Attention_3(Mamba)'
ISHOWN_PATH = './Train_results3_mamba'
ISHOWN_PATH_ = './Test_results3_mamba'
# LOSS_PATH = './Loss'
TRAIN_BATCH_SIZE = 2 
TEST_BATCH_SIZE = 1
...
T.load_state_dict(torch.load('/home/lily/model/GAN-Model/Task_Induced_Discriminator_save/20_TLoss0.6699_TrainACC0.6592_TestACC0.725_TestSEN0.7_TestSPE0.75_TestAUC0.7525_F1S0.7179.pth'))# best model 
...
for iteration in range(EPOCH):
    ...
    try:
        del x_fake, d_, t_, mri_images, pet_images, loss_g, real_y, fake_y
    except:
        pass 
    gc.collect()
    ...
    with torch.no_grad():
        for train_s in range(1):
            ...
            for val_trian_data in data_loader_valid:
                ...
                mri_images = Variable(mri_images.cuda()) 
                pet_images = val_trian_imgs[:, 1, :, :, :].view(val_trian_data_batch_size, 1, 76, 94, 76)
                pet_images = Variable(pet_images.cuda())
            ...
            ori_data = np.squeeze(mri_images.cpu().numpy())
            fake_data = np.squeeze(gen_pet_images.cpu().numpy())
            real_data = np.squeeze(pet_images.cpu().numpy())
    ...
    del mri_images, pet_images, gen_pet_images
    gc.collect()
    torch.cuda.empty_cache()
    ...
    for test_s in range(1):
        ...
        with torch.no_grad():
            for val_test_data in data_loader_test:
                ...
                mri_images_ = Variable(mri_images_.cuda())
                pet_images_ = val_test_imgs[:, 1, :, :, :].view(val_test_data_batch_size, 1, 76, 94, 76)
                pet_images_ = Variable(pet_images_.cuda())
