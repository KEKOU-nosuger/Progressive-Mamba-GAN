...
class MambaLayer3d(nn.Module):
    "Layernome -> Mamba -> Residual Connection"
    ...
    def forward(self,x):
        "x shape :[Batch,Channel,D,H,W]"

        B,C,D,H,W, = x.shape
        x_flat = x.flatten(2).tramspose(1,2)
        "变形 ->(b,l,c)"
        x_in = self.norm(x_flat)
